...
class StoikovStrategy_old:
    ...
    def update_reservation_price(self) -> None:
        time_to_terminal = (
            self.terminal_date - pd.to_datetime(self.current_time)
        ).delta / self.vol_freq

        self.reservation_price = (
            self.current_midprice
            - (self.coin_position / self.trade_size)
    ...
    def update_spread(self) -> None:
        ...
        self.spread = self.risk_preference * self.volatility * time_to_terminal + (
    ...
    def run(self, sim: Real_Data_Env) -> Tuple[
        ...
        while True:
            ...
            if self.current_time - prev_time >= self.delay:
                ...
                bid_price = round(
                    self.reservation_price - self.spread / 2, 1
                )  # increment
    ...
    def visualize_bids(quotes_history, freq=10):
        time, pos, bid, mid, reservation, ask = list(
            map(list, zip(*quotes_history[::freq]))
        )
        ...
        fig.add_trace(
            go.Scatter(x=df.index, y=df["bid"], name="bid"), secondary_y=False
        )

        fig.add_trace(
            go.Scatter(x=df.index, y=df["mid"], name="mid"), secondary_y=False
        )
        ...
        fig.add_trace(
            go.Scatter(x=df.index, y=df["ask"], name="ask"), secondary_y=False
        )

        fig.add_trace(go.Scatter(x=df.index, y=df["pos"], name="pos"), secondary_y=True)

        fig.update_layout(title_text="The mid-price and the optimal bid and ask quotes")
    ...
    def run(self, sim: Real_Data_Env) -> Tuple[
        ...
        while True:
            ...
            if self.current_time - prev_time >= self.delay:
                ...
                if (
                    self.parabola_a * inventory**2
                    + self.parabola_b * inventory
                    + self.parabola_c
                ) > spread:
                ...
                prev_time = self.current_time
                self.actions_history.append(
                    (self.current_time, self.coin_position, self.current_spread, action)
                )
...
class StoikovStrategyGeneralizedSingleAsset:
    ...
    def __init__(
        ...
        delay: float,
        hold_time: Optional[float] = None,
        risk_aversion: Optional[float] = 0,
        k: Optional[float] = 1,
        A: Optional[float] = 1,
    ) -> None:
        """
    ...
    def run(self, sim: Real_Data_Env) -> Tuple[
        ...
        while True:
            ...
            if receive_ts - prev_time >= self.delay:
                ...
                if xi != 0:
                    delta_ask = 1 / xi / delta_ * np.log(1 + xi * delta_ / k) - (
                        2 * q - delta_
                    ) / 2 * np.sqrt(
                        self.gamma
                        * (sigma**2)
                        ...
                        / delta_
                        / k
                        * (1 + xi * delta_ / k) ** (1 + k / xi / delta_)
                    )
                    delta_bid = 1 / xi / delta_ * np.log(1 + xi * delta_ / k) + (
                        2 * q + delta_
                    ) / 2 * np.sqrt(
                        self.gamma
                        * (sigma**2)
                    ...
                    delta_ask = 1 / k + (2 * q + delta_) / 2 * np.sqrt(
                        self.gamma * sigma**2 * np.e / (2 * A * delta_ * k)
                    )
                    delta_bid = 1 / k - (2 * q - delta_) / 2 * np.sqrt(
                        self.gamma * sigma**2 * np.e / (2 * A * delta_ * k)
                    )
                ...
                quotes_history.append(
                    (receive_ts, self.asset_position, bid_price, mid_price, ask_price)
                )
